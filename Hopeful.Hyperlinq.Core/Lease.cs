...
{
    private static class EmptyLease<T>
    {
        // ReSharper disable once InconsistentNaming
        internal static readonly Lease<T> Value = new(ArrayPool<T>.Shared, 0, default);
    }

    public static Lease<T> Empty<T>() => EmptyLease<T>.Value;
}

public sealed class Lease<T>: IMemoryOwner<T>, IValueEnumerable<T, Lease<T>.DisposableEnumerator>
{
    ...
    internal Lease(ArrayPool<T> pool, int length, bool clearOnDispose)
    {
        ...
        rented = length is 0 ? [] : pool.Rent(length);
    }
    ...
    public struct DisposableEnumerator: IEnumerator<T>
    {
        ...
        readonly object? IEnumerator.Current => source[index]; // ReSharper disable once HeapView.PossibleBoxingAllocation
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Lease<T> AsEnumerable() => this;
}
